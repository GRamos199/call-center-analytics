...
class AgentContent(BaseContent):
    ...
    def render(self, selected_period: Any, previous_period: Any = None) -> None:
        ...
        try:
            ...
            self._render_efficiency_rankings(filtered_df, selected_period, latest_period)
    ...
    def _filter_by_agent(self, agent_df: pd.DataFrame, selected_agent: str) -> pd.DataFrame:
    ...
    def _render_agent_table(self, agent_df: pd.DataFrame, selected_period: Any, latest_period: Any) -> None:
        ...
        # Format columns
        display_df["resolution_rate"] = (display_df["resolution_rate"] * 100).round(2).astype(str) + "%"
        display_df["avg_handle_time_minutes"] = display_df["avg_handle_time_minutes"].round(1).astype(str) + " min"
        display_df["customer_satisfaction_score"] = display_df["customer_satisfaction_score"].round(2)
        display_df["hours_worked"] = display_df["hours_worked"].round(1).astype(str) + " hrs"
    ...
    def _render_department_donuts(self, agent_df: pd.DataFrame, selected_period: Any, latest_period: Any) -> None:
        ...
        dept_summary = period_df.groupby("department").agg({
    ...
    def _render_efficiency_rankings(self, agent_df: pd.DataFrame, selected_period: Any, latest_period: Any) -> None:
